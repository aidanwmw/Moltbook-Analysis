...
- data_exploration.ipynb : This file is an overview of graph-level statistics for our own research out of class (for example, fitting power-law distributions and looking at the change in time of the topology). More network theoretic stuff than ML and was not used for classification, but was useful for our paper's introduction so we included it. 

- Random_Forest.py : This file is the random forest classifier for the moltbook vs. reddit ego graph classification task.
